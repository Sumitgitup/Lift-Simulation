...
function generateFloors() {
  ...
  for (let i = totalFloors; i >= 1; i--) {
    ...
    if (i === totalFloors) {
      upBtn.style.display = 'none'; // Hide the UP button on the top floor
    } else if (i === 1) {
      downBtn.style.display = 'none'; // Hide the DOWN button on the ground floor
    }
...
function buttonClickHandler(event) {
  ...
  // Check if any lift is already present at the destination floor
  const { liftElement, liftId } = checkIsLiftAlreadyPresent(destinationFloor);

  if (!liftElement) { // Only move a lift if no lift is already at the destination
    const availableLift = getAvailableLift(destinationFloor); // Get the nearest available lift
    if (availableLift) {
      moveLift(availableLift, destinationFloor); // Move the lift to the destination floor
    } else {
      pendingFloors.push(destinationFloor); // If no lift is available, add the request to the queue
    }
  }
}
